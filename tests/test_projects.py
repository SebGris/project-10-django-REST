...
class TestProjects:
    ...
    def test_list_only_contributor_projects(
        self, authenticated_client, create_project, create_user
    ):
    ...
    def test_add_contributor_twice(
        self, authenticated_client, create_project, create_user
    ):
    ...
    def test_non_author_cannot_add_contributor(
        self, authenticated_client, create_project, create_user
    ):
    ...
    def test_contributor_cannot_update_project(
        self, authenticated_client, create_project, create_user
    ):
...
class TestIssues:
    ...
    def test_cannot_assign_to_non_contributor(
        self, authenticated_client, create_project, create_user
    ):
    ...
    def test_update_issue_by_author(
        self, authenticated_client, create_project, create_issue
    ):
        ...
        url = reverse(
            "project-issues-detail", kwargs={"project_pk": project.id, "pk": issue.id}
        )
    ...
    def test_project_author_can_update_any_issue(
        self, authenticated_client, create_project, create_user, create_issue
    ):
    ...
    def test_contributor_cannot_update_others_issue(
        self, authenticated_client, create_project, create_user, create_issue
    ):
    ...
    def test_list_project_issues(
        ...
        for i in range(3):
            create_issue(
                name=f"Issue {i}", project=project, author=authenticated_client.user
            )
    ...
    def test_non_contributor_cannot_see_issues(
        self, authenticated_client, create_project, create_user, create_issue
    ):
...
class TestComments:
    ...
    def test_create_comment(self, authenticated_client, create_project, create_issue):
        ...
        url = reverse(
            "issue-comments-list",
            kwargs={"project_pk": project.id, "issue_pk": issue.id},
        )
    ...
    def test_update_comment_by_author(
        ...
        url = reverse(
            ...
            kwargs={"project_pk": project.id, "issue_pk": issue.id, "pk": comment.id},
        )
    ...
    def test_delete_comment_by_project_author(
        self, authenticated_client, create_project, create_user, create_issue
    ):
        ...
        issue = create_issue(project=project, author=authenticated_client.user)
        comment = Comment.objects.create(
            description="Comment by other user", issue=issue, author=other_user
        )
    ...
    def test_contributor_cannot_delete_others_comment(
        self, authenticated_client, create_project, create_user, create_issue
    ):
        ...
        issue = create_issue(project=project, author=author)
        comment = Comment.objects.create(
            description="Comment by project author", issue=issue, author=author
        )
    ...
    def test_non_contributor_cannot_comment(
        self, authenticated_client, create_project, create_user, create_issue
    ):
...
class TestPermissions:
    ...
    def test_issue_priorities_and_tags(self, authenticated_client, create_project):
        ...
        issue_statuses = [
            ...
            "In Progress",
            "Finished",
        ]  # Renommé pour éviter confusion

        for i, (priority, tag, issue_status) in enumerate(
            zip(priorities, tags, issue_statuses)
        ):
            data = {
                "name": f"Issue {i}",
